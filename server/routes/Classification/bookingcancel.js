...
router.post('/booking-cancel-register', async (req, res) => {
    try {
      ...
      const existing = await bookingCancelProfile.find({ booking_id });
      if (existing.length > 0) {
        return res.status(400).json({ message: "Booking already cancelled" });
      }

    //   const assigned = await bookingassistantProfile.find({ booking_id });
    //   if (assigned.length > 0) {
    //     return res.status(400).json({ message: "Canceled Task.. Can't Assigned." });
    //   }
      ...
      const bookingCancel = new bookingCancelProfile({
        booking_id,
        cancel_reason,
      });
      await bookingCancel.save();
      ...
      const assistantCount = await bookingassistantProfile.countDocuments({ booking_id });
      if (assistantCount > 0) {
        await bookingassistantProfile.deleteMany({ booking_id });
      }
