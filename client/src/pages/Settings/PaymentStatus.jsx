...
export default function NationalitySetting() {
  ...
  const handleModalSubmit = async (formData) => {
    try {
      const res = await fetch("/api/payment-status-register", {
        method: "POST",
        headers: {
      ...
      window.location.href = "/settings/paymentstatus";
    } catch (error) {
  ...
  const handleEditModalSubmit = async (updatedData) => {
    ...
    try {
      const response = await fetch(
        `/api/payment-status-profile/${selectedId}`,
        {
          method: "PUT",
          headers: {
  ...
  const handleDelete = async (id) => {
    try {
      const res = await fetch(`/api/payment-status-profile/${id}`, {
        method: "DELETE",
      });
      ...
      <Layout>
        ...
        <Breadcrumb
          items={[
            ...
            { label: "Settings", link: "#" },
            { label: "Payment Status", link: "/settings/paymentstatus" },
          ]}
        />
        {loading ? (
          <div>Loading...</div>
