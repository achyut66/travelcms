...
export default function NationalitySetting() {
  ...
  const handleModalSubmit = async (formData) => {
    try {
      const res = await fetch("/api/visit-purpose-register", {
        method: "POST",
        headers: {
      ...
      window.location.href = "/settings/visitpurpose";
    } catch (error) {
  ...
  const handleEditModalSubmit = async (updatedData) => {
    ...
    try {
      const response = await fetch(`/api/visit-purpose-profile/${selectedId}`, {
        method: "PUT",
        headers: {
  ...
  const handleEdit = async (id) => {
    setSelectedId(id);
    try {
      const res = await fetch(`/api/visit-purpose-profile/${id}`);
      const data = await res.json();
      ...
      <Layout>
        ...
        <Breadcrumb
          items={[
            ...
            { label: "Settings", link: "#" },
            { label: "Visit Purpose", link: "/settings/visitpurpose" },
          ]}
        />
        {loading ? (
          <div>Loading...</div>
